...
class SendPolicyCustomerController extends NovaController {
	
    protected $module_path='reception/newPolicy/sendPolicyCustomer';
    ...
    public function printGuia($process_ID,Request $request){
        try{
            $pup=ProcessSendPolicyCustomer::findProcess($process_ID);
    ...
    public function getDataGuia($pup,$request){
        ...
        $data['reason']='send_policy_customer';
    ...
    private function renderForm($method,$url,$id,$carries,$data){
        ...
        $form['url']=$url;
        $form['files']=true;
        $form["title"]="Registrar envío de póliza al cliente";
        $form['sections'][0]=[
