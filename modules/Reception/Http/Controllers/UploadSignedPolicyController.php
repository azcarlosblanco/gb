...
class UploadSignedPolicyController extends NovaController {
	
    protected $module_path='reception/newPolicy/uploadSignedPolicy';
	...
	public function upload($process_ID, Request $request){
		\DB::beginTransaction();
		try{
            $pup=ProcessUploadSignedPolicy::findProcess($process_ID);
    		...
    		$this->novaMessage->setRoute(
    				'reception/pending');
    		$this->novaMessage->addSuccesMessage('Finalizado','Trámite ha sido completado');
	...
	private function renderForm($method,$url){
        ...
        $form['url']=$url;
        $form['files']=true;
        $form["title"]="Subir Póliza Firmada";
        $form['sections'][0]=[
                            'fields' => array(
                                array(
                                        'label'    => 'Cargar Poliza',
