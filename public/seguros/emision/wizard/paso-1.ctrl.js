angular
.module('ngNova')
.controller('WizPaso1Ctrl', ['response','Restangular','$scope', 'hotkeys', '$state', '$http', '__env', 'webNotification', 
	function(response, Restangular, $scope, hotkeys, $state, $http, __env, webNotification){

	console.log(response); //      //      //      //      //      //      //      //      //
	var temp_data = JSON.parse(response.data.data.temp_data);
	...
	$scope.form = temp_data.customer_obj;

	if($scope.form.weightu == undefined){
		$scope.form.weightu = "lb";
	}

	if($scope.form.heightu == undefined){
		$scope.form.heightu = "cm";
	}
	...
	$scope.next = function () {
		if ($scope.form) {
			...
			var	postUrl = "emission/newPolicy/uploadPolicyRequest/form/"+process_ID+"/step/1";
