...
.controller('NewConceptCtrl', 
        ...
        $scope.closeAddConcept = function(){
        	myShareService.broadcastCancelConcept();
        }

        $scope.transmitConcept = function(conceptObject) {
        	myShareService.prepForBroadcastConcept(conceptObject);
	    };
			...
			.then(
				function successCallback(response) {
					...
					var concept = {"id":id,
									...
									"notify":$scope.form.notify,
									"deduct_discount":$scope.form.deduct_discount,
									"showmodal":false}
					notificacion = {'title':'Creación exitosa',
									'body':'El concepto fue creado exitosamente'}
					$scope.showNotification(notificacion);
					$scope.transmitConcept(concept);
				},
